...
CORS_ALLOWED_ORIGINS = [x.strip().rstrip("/") for x in os.getenv("CORS_ALLOWED_ORIGINS","http://localhost:5173,https://biavanta-frontend.onrender.com").split(",") if x.strip()]
CSRF_TRUSTED_ORIGINS = [x.strip().rstrip("/") for x in os.getenv("CSRF_TRUSTED_ORIGINS","http://localhost:5173,https://biavanta-frontend.onrender.com").split(",") if x.strip()]
